-- Additive US5 roster command slice. Depends on 20260729003000.
...
begin
  ...
  if found then
    ...
    return jsonb_build_object('replayed',true,'bookingId',v_booking.workshop_booking_id,
  ...
  if p_action='create_reservation' then
    ...
    if v_quantity<=0 or v_mode not in ('manual','complimentary')
    ...
    insert into public.workshop_bookings(
      ...
      contact_name,contact_email,contact_phone,purchased_quantity,active_quantity,status,
      payment_state,price_per_seat_minor_snapshot,subtotal_minor_snapshot,total_minor_snapshot,
      required_charges_minor_snapshot,currency,terms_snapshot,terms_version,confirmed_at
    ) values (
      v_occurrence.workshop_occurrence_id,'BBW-'||to_char(v_occurrence.start_at,'YYYY')||'-'
    ...
    insert into public.workshop_seat_holds(
      ...
      v_occurrence.workshop_occurrence_id,v_booking.workshop_booking_id,v_quantity,
      'confirmed',null,v_occurrence.end_at,v_occurrence.end_at,now(),v_mode,p_command_key
    );
    ...
    return jsonb_build_object('replayed',false,'bookingId',v_booking.workshop_booking_id,
      'status',v_booking.status,'activeQuantity',v_booking.active_quantity);
    ...
    update public.workshop_bookings set active_quantity=v_new_active,
    ...
    if v_new_active=0 then
      update public.workshop_seat_holds set state='cancelled',
        resolved_at=coalesce(resolved_at,now()),resolution_reason='booking_cancelled'
    ...
    update public.workshop_attendees set attendance_state='checked_in',
      checked_in_at=coalesce(checked_in_at,now()),checked_in_by=coalesce(checked_in_by,auth.uid())
    where workshop_attendee_id=v_attendee.workshop_attendee_id returning * into v_attendee;
    update public.workshop_bookings set status='checked_in',
    ...
    insert into public.workshop_audit_events(
      ...
      v_booking.workshop_occurrence_id,'attendee_checked_in','internal',auth.uid(),p_command_key,
      jsonb_build_object('bookingId',v_attendee.workshop_booking_id,
        'attendeeId',v_attendee.workshop_attendee_id,'attendanceState','checked_in')
    );
    return jsonb_build_object('replayed',false,'bookingId',v_attendee.workshop_booking_id,
      'attendeeId',v_attendee.workshop_attendee_id,'attendanceState','checked_in');
  else
...
returns table(booking_reference text,contact_name text,active_quantity integer,
  booking_status text,seat_number smallint,attendee_name text,attendance_state text)
language sql stable security definer set search_path='' as $$
  ...
  from public.workshop_bookings b join public.workshop_attendees a using(workshop_booking_id)
  where public.is_internal_crm_user() and b.workshop_occurrence_id=p_occurrence_id
