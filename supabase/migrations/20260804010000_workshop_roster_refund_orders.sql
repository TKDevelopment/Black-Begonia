alter table public.workshop_refund_requests
  add column if not exists seat_quantity integer null
    check (seat_quantity > 0),
  add column if not exists seats_released_at timestamptz null;
...
begin
  ...
  if found then
    ...
    return jsonb_build_object(
      'replayed', true, 'bookingId', v_booking.workshop_booking_id,
...
begin
  ...
  where workshop_payment_transaction_id =
    nullif(p_payload->>'transactionId', '')::uuid for update;
  ...
  then raise exception 'refund_not_eligible' using errcode = 'P0001'; end if;
  ...
  if p_action = 'eligibility' then
    ...
    return jsonb_build_object(
      ...
      'currency', v_charge.currency, 'provider', v_charge.provider,
  ...
  if p_action = 'request_stripe' then
    ...
    if found then
      return jsonb_build_object(
        'replayed', true, 'requestId', v_request.workshop_refund_request_id,
        'state', v_request.state, 'amountMinor', v_request.amount_minor,
        'seatQuantity', v_request.seat_quantity, 'currency', v_request.currency,
        'providerChargeId', v_charge.provider_transaction_id,
    ...
    v_result := public.manage_workshop_financials(
      ...
      jsonb_build_object(
        'transactionId', v_charge.workshop_payment_transaction_id,
        'amountMinor', v_amount, 'currency', v_charge.currency,
        'reason', v_reason
      ), p_command_key
    );
  ...
  if p_action = 'record_venmo' then
    ...
    then raise exception 'refund_not_eligible' using errcode = 'P0001'; end if;
    v_result := public.manage_workshop_financials(
      'record_external_refund', p_payload - 'seatQuantity', p_command_key
    );
    perform public.apply_workshop_refund_seat_release(
      v_charge.workshop_booking_id, v_seat_quantity, v_amount,
...
begin
  ...
  then
    perform public.apply_workshop_refund_seat_release(
      new.workshop_booking_id, new.seat_quantity, new.amount_minor,
      'Stripe refund confirmed', new.workshop_refund_request_id,
      'provider', null
    );
...
from public, anon, authenticated;
drop trigger if exists trg_workshop_refund_requests_release_seats
on public.workshop_refund_requests;
