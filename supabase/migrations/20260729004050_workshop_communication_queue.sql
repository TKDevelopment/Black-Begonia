-- Additive US5 durable communication queue. Immutable communication rows remain
-- terminal delivery facts; mutable claims and retries live in separate tables.
...
create table public.workshop_message_queue(
  ...
  workshop_personal_data_request_id uuid references public.workshop_personal_data_requests(workshop_personal_data_request_id) on delete restrict,
  ...
  token_purpose text check(token_purpose in(
    'status_access','waitlist_offer','reschedule_response',
    'privacy_verification','replacement_email')),
  template_version text not null,is_required boolean not null default true,
  state text not null default 'queued' check(state in(
    'queued','claimed','sent','failed','suppressed')),
  attempt_count integer not null default 0 check(attempt_count between 0 and 10),
  next_attempt_at timestamptz not null default now(),expires_at timestamptz,
  claimed_at timestamptz,claimed_by text,provider_message_id text,
  last_error_category text,command_key uuid not null unique,
  token_registration_key uuid unique,
  outcome_command_key uuid unique,created_at timestamptz not null default now(),
...
on public.workshop_message_queue(state,next_attempt_at,created_at) where state='queued';
alter table public.workshop_message_queue enable row level security;
create policy workshop_message_queue_internal_select on public.workshop_message_queue
...
create table public.workshop_message_outcomes(
  ...
  error_category text,command_key uuid not null unique,
  created_at timestamptz not null default now()
);
alter table public.workshop_message_outcomes enable row level security;
create policy workshop_message_outcomes_internal_select on public.workshop_message_outcomes
for select to authenticated using(public.is_internal_crm_user());
...
create or replace function public.queue_workshop_communication(
  p_communication_type text,p_recipient_source text,p_template_version text,
  p_booking_id uuid,p_waitlist_entry_id uuid,p_personal_data_request_id uuid,
  p_token_purpose text,p_is_required boolean,p_expires_at timestamptz,p_command_key uuid
...
begin
  ...
  ) values(p_booking_id,p_waitlist_entry_id,p_personal_data_request_id,
...
) returns table(queue_id uuid,communication_type text,recipient_email text,
  token_purpose text,template_version text,personal_data_request_id uuid,
  waitlist_entry_id uuid,booking_id uuid,expires_at timestamptz,attempt_count integer)
...
begin
  ...
  select u.workshop_message_queue_id,u.communication_type,
    case u.recipient_source when 'booking_contact' then b.contact_email
      when 'waitlist_contact' then w.contact_email
      when 'proposed_correction_email'
        then lower(btrim(r.protected_proposed_corrections->>'contact_email')) end,
  ...
  from updated u left join public.workshop_bookings b
    on b.workshop_booking_id=u.workshop_booking_id
...
declare
  v_queue public.workshop_message_queue;
  v_existing public.workshop_message_outcomes;
  v_booking_id uuid;
  v_state text;
begin
  select * into v_existing from public.workshop_message_outcomes where command_key=p_command_key;
  if found then return jsonb_build_object('replayed',true,
    'state',v_existing.resulting_state,'attemptCount',v_existing.attempt_count); end if;
  select * into v_queue from public.workshop_message_queue
  where workshop_message_queue_id=p_queue_id for update;
  ...
  if p_outcome='retryable' and v_queue.attempt_count<5 then
    update public.workshop_message_queue set state='queued',
      ...
      last_error_category=coalesce(nullif(p_error_category,''),'provider_retryable'),
  ...
  ) values(v_booking_id,v_queue.workshop_waitlist_entry_id,v_queue.communication_type,
    coalesce(nullif(p_recipient_digest,''),'unavailable'),v_queue.template_version,
    coalesce(nullif(p_payload_digest,''),'unavailable'),nullif(p_provider_message_id,''),
    v_state,now(),p_command_key);
...
revoke all on function public.queue_workshop_communication(
  text,text,text,uuid,uuid,uuid,text,boolean,timestamptz,uuid) from public;
...
revoke all on function public.record_workshop_communication_outcome(
  uuid,text,text,text,text,text,text,timestamptz,uuid) from public;
...
grant execute on function public.queue_workshop_communication(
  text,text,text,uuid,uuid,uuid,text,boolean,timestamptz,uuid) to service_role;
...
grant execute on function public.record_workshop_communication_outcome(
  uuid,text,text,text,text,text,text,timestamptz,uuid) to service_role;
grant execute on function public.complete_workshop_message_delivery(
  uuid,text,text,text,text,text,uuid) to service_role;
grant execute on function public.cancel_workshop_communications(uuid,uuid,text) to service_role;
...
after insert on public.workshop_bookings
