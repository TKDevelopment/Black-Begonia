create or replace function public.get_public_workshop_occurrence_catalog(p_slug text)
returns jsonb language sql stable security definer set search_path = ''
as $$
  select jsonb_build_object(
    'slug', o.slug, 'title', o.title_snapshot,
    ...
    'availability', case
      when o.status <> 'published_open' or o.registration_closes_at <= now() then 'closed'
      else 'available'
    end,
    ...
    'replacementUrl', case when replacement.slug is not null
      then '/workshops/' || replacement.slug else null end,
    ...
    'redirectUrl', case when o.status_page_expires_at <= now()
      then coalesce('/workshops/' || replacement.slug, '/workshops') else null end,
    'media', coalesce((
...
as $$
  select o.slug, o.updated_at, o.status::text,
