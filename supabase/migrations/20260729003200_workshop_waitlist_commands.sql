-- Additive US5 waitlist command slice. Depends on 03100 and precedes privacy.
alter table public.workshop_waitlist_offers
add column workshop_seat_hold_id uuid references public.workshop_seat_holds(workshop_seat_hold_id) on delete restrict,
add column resolution_command_key uuid unique;
-- No offers exist before this unpromoted feature slice is enabled.
alter table public.workshop_waitlist_offers alter column workshop_seat_hold_id set not null;
...
begin
  ...
  select * into v_entry from public.workshop_waitlist_entries where command_key=p_command_key;
  if found then return jsonb_build_object('replayed',true,'state',v_entry.state,
    'entryId',v_entry.workshop_waitlist_entry_id); end if;
  ...
  if found then return jsonb_build_object('replayed',true,'state',v_offer.state,
    'entryId',v_offer.workshop_waitlist_entry_id,'offerId',v_offer.workshop_waitlist_offer_id,
    'quantity',v_offer.quantity,'effectiveExpiresAt',v_offer.expires_at); end if;
  ...
  if p_action='join' then
    ...
    ) values(v_occurrence.workshop_occurrence_id,btrim(p_payload->>'contactName'),
    ...
    where e.state='offered' and exists(
      ...
      where o.workshop_waitlist_entry_id=e.workshop_waitlist_entry_id and o.state='expired'
    );
    ...
    where workshop_occurrence_id=v_occurrence.workshop_occurrence_id and state='waiting'
    order by created_at,workshop_waitlist_entry_id limit 1 for update skip locked;
    ...
    ) values(v_occurrence.workshop_occurrence_id,v_quantity,'active',null,
    ...
    ) values(v_entry.workshop_waitlist_entry_id,v_occurrence.workshop_occurrence_id,
      v_hold.workshop_seat_hold_id,v_quantity,
      coalesce(nullif(p_payload->>'offerTokenDigest',''),
        repeat(md5(gen_random_uuid()::text),2)),
      v_expires,p_command_key) returning * into v_offer;
    ...
    return jsonb_build_object('replayed',false,'state','active',
      'entryId',v_entry.workshop_waitlist_entry_id,'offerId',v_offer.workshop_waitlist_offer_id,
      'quantity',v_quantity,'effectiveExpiresAt',v_expires);
    ...
    ) values(v_occurrence.workshop_occurrence_id,
