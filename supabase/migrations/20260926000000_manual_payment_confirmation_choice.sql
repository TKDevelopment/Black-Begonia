create or replace function public.record_manual_payment(
  ...
  p_note text,
  p_send_confirmation_email boolean,
...
declare
  ...
  v_send_confirmation_email boolean := coalesce(p_send_confirmation_email, true);
...
begin
  ...
  insert into public.payment_transactions(
    ...
    p_suspected_reference,
    case when v_send_confirmation_email then 'required' else 'none' end,
...
revoke all on function public.record_manual_payment(uuid,uuid,bigint,text,timestamptz,text,boolean,text,text,uuid,boolean,boolean) from public, anon;
grant execute on function public.record_manual_payment(uuid,uuid,bigint,text,timestamptz,text,boolean,text,text,uuid,boolean,boolean) to authenticated;

-- Preserve existing clients: calls without the notice choice continue to queue a receipt.
create or replace function public.record_manual_payment(
  p_project_id uuid,
  p_obligation_id uuid,
  p_amount_cents bigint,
...
as $$
  select public.record_manual_payment(
    p_project_id, p_obligation_id, p_amount_cents, p_method, p_received_at, p_note,
    true, p_suspected_reference, p_override_reason, p_command_key,
    p_confirm_overpayment, p_confirm_spillover
  );
$$;
