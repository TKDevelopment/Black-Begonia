-- Make Stripe's multiple success-event variants converge on one charge fact.
...
begin
  ...
  insert into public.workshop_payment_provider_events (
    ...
    p_event_type, p_event_occurred_at, p_signature_verified_at,
    p_payload_digest,
    jsonb_build_object(
  ...
  if not found then
    ...
    insert into public.workshop_payment_exceptions (
      workshop_occurrence_id, exception_type, urgency, amount_minor,
      currency, summary, safe_detail, provider_event_id, command_key
    ) values (
      null, 'unmatched_payment', 'urgent', p_amount_minor, p_currency,
  ...
  if p_event_type in (
    ...
    if v_attempt.state = 'paid' or v_booking.payment_state = 'paid' then
      ...
      where workshop_payment_provider_event_id =
        v_event.workshop_payment_provider_event_id;
      return jsonb_build_object(
        'replayed', false, 'state', 'duplicate'
      );
    end if;
    ...
    set status = 'expired', payment_state = 'exception'
  ...
  -- Checkout Session and PaymentIntent events can represent the same charge.
  -- Resolve that semantic duplicate before evaluating booking state or inserting.
  ...
  elsif v_exception_type is null
    and v_hold.state not in ('active', 'expired')
  then
  ...
  insert into public.workshop_payment_transactions (
    ...
    v_booking.workshop_booking_id, v_occurrence.workshop_occurrence_id,
    v_attempt.workshop_payment_attempt_id, 'charge', 'stripe',
    p_provider_payment_id, 'STRIPE-' || p_provider_event_id,
    p_amount_minor, p_currency, p_event_occurred_at,
    case when v_exception_type is null then 'paid' else 'exception' end,
    p_command_key, p_payload_digest,
  ...
  if v_exception_type is null then
    ...
    where workshop_seat_hold_id = v_hold.workshop_seat_hold_id;
    update public.workshop_bookings
    set status = 'confirmed', payment_state = 'paid',
    ...
    insert into public.workshop_payment_exceptions (
      workshop_booking_id, workshop_occurrence_id,
      workshop_payment_attempt_id, workshop_payment_transaction_id,
      ...
      v_transaction.workshop_payment_transaction_id,
      v_exception_type, 'urgent', p_amount_minor, p_currency,
  ...
  return jsonb_build_object(
    'replayed', false,
    'state', case
      when v_exception_type is null then 'confirmed'
      else 'exception'
    end,
...
-- Rollback: restore the preceding function definition only after workshop
-- checkout is disabled and all received Stripe events are reconciled.
