-- Additive workshop financial operations companion.
-- Depends on the base payments/reconciliation slice and preserves all history.

alter table public.workshop_payment_transactions
  drop constraint workshop_payment_transactions_transaction_type_check;
alter table public.workshop_payment_transactions
  add constraint workshop_payment_transactions_transaction_type_check
  check (transaction_type in (
    'charge','refund','external_refund','fee','discount','dispute','reversal',
    'adjustment','correction'
  ));
alter table public.workshop_payment_transactions
  add column actor_type text not null default 'provider'
    check (actor_type in ('provider','internal','system')),
  add column actor_id uuid null references public.profiles(id) on delete set null,
  add column note text null;

alter table public.workshop_expenses
  alter column workshop_occurrence_id drop not null,
  alter column amount_minor drop default,
  drop constraint workshop_expenses_amount_minor_check;
alter table public.workshop_expenses
  add column workshop_series_id uuid null
    references public.workshop_series(workshop_series_id) on delete restrict,
  add column vendor_or_payee text null,
  add column note text null,
  add column correction_of_expense_id uuid null
    references public.workshop_expenses(workshop_expense_id) on delete restrict,
  add column is_reversal boolean not null default false,
  add constraint workshop_expenses_amount_minor_check check (amount_minor > 0),
  add constraint workshop_expenses_expense_category_check check (
    expense_category in (
      'materials','venue','labor','marketing','travel','equipment','fees','other'
    )
  ),
  add constraint workshop_expense_scope check (
    (workshop_occurrence_id is not null)::integer
      + (workshop_series_id is not null)::integer = 1
  );
create index idx_workshop_expenses_series
on public.workshop_expenses (workshop_series_id, incurred_on desc);
create unique index uq_workshop_expense_reversal
on public.workshop_expenses (correction_of_expense_id)
where is_reversal;

drop view public.workshop_financial_entries;
