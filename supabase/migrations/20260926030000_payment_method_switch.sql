-- Keep customer payment methods switchable while preserving one reminder pause.
-- Only offer Venmo when the approved business-profile target is usable.
...
begin
  ...
  if p_venmo_enabled and btrim(p_venmo_business_target) !~* '^(@[A-Za-z0-9_-]{2,64}|https://(www\.)?venmo\.com/u/[A-Za-z0-9_-]{2,64}/?)$' then raise exception 'Enter a Venmo @handle or venmo.com/u/ profile URL'; end if;
