  ...
  if p_action='create_request' then
    ...
    if not found then
      ...
      if coalesce(p_payload->>'bookingStatusDigest','')=v_booking.status_token_digest
        and v_booking.status_token_expires_at>now()
      then
    ...
    if not found or v_request.verification_expires_at<=now() then
      if found then
        update public.workshop_personal_data_requests set state='expired',
    ...
    if not found or v_request.replacement_email_expires_at<=now() then
      ...
      update public.workshop_personal_data_requests set
        verification_token_digest=null,replacement_email_token_digest=null,
      ...
      insert into public.workshop_audit_events(
        ...
        v_occurrence_id,'personal_data_replacement_email_confirmed','customer',
        p_command_key,jsonb_build_object('outcome','completed')
      );
    ...
    if p_payload->>'decision'='deny' then
      ...
      if 'contact_email'=any(v_request.requested_field_categories) then
        ...
        select max(deadline) into v_earliest from (
          values(v_booking.status_token_expires_at),
            (v_email_policy_earliest),
            ((select max(response_token_expires_at)
              from public.workshop_reschedule_responses
              where workshop_booking_id=v_booking.workshop_booking_id and response='pending')),
            ((select max(verification_expires_at)
              from public.workshop_personal_data_requests
              where workshop_booking_id=v_booking.workshop_booking_id
                and verification_token_digest is not null))
        ) d(deadline);
        if v_email_blocked then
          v_state:='deferred';
        else
      ...
      insert into public.workshop_communication_suppressions(
        ...
        v_booking.workshop_booking_id,'verified_minimization',p_command_key,auth.uid()
      );
      perform public.cancel_workshop_communications(
    ...
    insert into public.workshop_audit_events(
      ...
      v_occurrence_id,'personal_data_request_processed','internal',auth.uid(),
